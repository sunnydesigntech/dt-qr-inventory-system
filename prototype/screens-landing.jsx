...
const ModeBadge = ({ mode }) => {
  if (mode === 'tech') return <span className="pill" style={{background:'#fffbeb', color:'#92400e', border:'1px solid #fde68a'}}><I name="edit" size={11} stroke={2.4}/>Technician</span>;
  return <span className="pill pill-blue"><I name="search" size={11} stroke={2.4}/>View</span>;
};
...
const LandingScreen = ({ data, onOpenLocation, tweaks }) => {
      ...
      <div className="bg-white border-b border-slate-200 px-4 pt-4 pb-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button className="tap rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-600" title="Diagnostics"><I name="info" size={20}/></button>
          </div>
        </div>
      </div>
          ...
          return (
            <section key={rcode}>
              ...
              <div className="space-y-2">
                {locs.map(l => <LocationCard key={l.id} loc={l} onOpen={()=>onOpenLocation(l, 'view')} onTech={()=>onOpenLocation(l, 'tech')}/>)}
              </div>
            </section>
          );
        })}
...
const LocationCard = ({ loc, onOpen, onTech }) => {
  ...
  return (
    <article className="card rounded-2xl overflow-hidden">
      ...
      <div className="border-t border-slate-100 px-2 py-1.5 flex gap-1">
        ...
        <button onClick={onTech} className="flex-1 py-2 rounded-lg text-[12.5px] font-semibold text-blue-700 hover:bg-blue-50 flex items-center justify-center gap-1.5"><I name="edit" size={13}/>Update</button>
      </div>
    </article>
  );
};

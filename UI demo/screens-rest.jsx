// View Mode + Update Mode + Empty + Error screens
    ...
    <article className={`card-flat rounded-2xl border bg-white p-3.5 ${stripe ? 'hazard-stripe border-red-200' : 'border-slate-200'} ${band ? 'hazard-band' : ''}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          {item.hazard && hazardTone !== 'section' && (
            <div className="mt-2 rounded-lg bg-red-50 border border-red-100 px-2.5 py-1.5 text-[11.5px] text-red-800 flex gap-1.5">
              ...
              <span>{item.safetyNote || 'Handle per D&T chemical storage and safety procedures.'}</span>
            </div>
          )}
          {/* meta chips: SDS, reorder level, maintenance due, supplier */}
          {(item.sds || item.reorderLevel != null || item.maintenanceDue || item.supplier || item.assetValue) && (
            <div className="mt-2 flex flex-wrap gap-1.5">
              {item.sds && <a href={item.sds} className="pill pill-blue hover:underline"><II name="external" size={10}/>SDS</a>}
              {item.reorderLevel != null && <span className="pill pill-neutral">Reorder ≤ {item.reorderLevel}</span>}
              {item.maintenanceDue && <span className="pill pill-maint"><II name="cpu" size={10}/>Due {item.maintenanceDue}</span>}
              {item.supplier && <span className="pill pill-neutral">{item.supplier}{item.cost ? ` · ${item.cost}` : ''}</span>}
              {item.assetValue && <span className="pill pill-neutral">{item.assetValue}</span>}
            </div>
          )}
        </div>
        ...
        {mode === 'view' ? (
          ...
          <UpdateControls item={item} onChange={onChange}/>
        )}
      </div>
    </article>
  );
...
const UpdateControls = ({ item, onChange }) => {
  return (
...
const ContextStrip = ({ loc, room, mode, onBack, onToggleMode }) => (
  <div className={`px-4 pt-3 pb-3 border-b border-slate-200 ${mode==='update' ? 'bg-amber-50/40' : 'bg-white'}`}>
    ...
    <div className="mt-2.5 flex flex-wrap items-center gap-1.5">
      ...
      <button onClick={onToggleMode} className="ml-auto pill pill-blue hover:bg-blue-100" style={{padding:'5px 10px'}}>
        ...
        {mode==='view' ? 'Switch to Update Mode' : 'Switch to View Mode'}
      </button>
    </div>
  </div>
);
...
const LocationScreen = ({ data, loc, mode, onBack, onToggleMode, tweaks }) => {
  ...
  return (
    <div className={`min-h-full ${mode==='update' ? 'mode-update-bg' : ''}`}>
      ...
      {/* item list */}
      <div className={`px-4 pt-3 ${mode==='update' ? 'pb-32' : 'pb-8'} space-y-2.5`}>
  ...
  <div className="min-h-full">
    ...
    <div className="px-4 pt-8">
      <div className="card rounded-2xl p-6 text-center">
        ...
        {mode === 'update' && <button className="btn btn-primary mt-4 mx-auto"><II name="plus" size={15}/> Add first item</button>}
      </div>
    </div>
  </div>
);
...
const ErrorScreen = ({ kind, onBack }) => {
  const variants = {
    ...
    'no-bridge':      { title: 'Save bridge unavailable', body: 'google.script.run is not available in this context. Read-only display still works.', fix: 'Open the deployed /exec URL directly to use Update Mode.', tone:'amber' },
  };

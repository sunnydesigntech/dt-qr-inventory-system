...
const App = () => {
  ...
  const toggleMode = () => setRoute(r => ({ ...r, mode: r.mode === 'view' ? 'update' : 'view' }));

  const screen = (() => {
    if (route.name === 'landing') return <window.LandingScreen data={data} onOpenLocation={goLocation} onNav={(name)=>setRoute({name})} tweaks={tweaks}/>;
    ...
    if (route.name === 'storage') return <window.StorageMaster data={data} onBack={goLanding}/>;
    if (route.name === 'audit') return <window.AuditLog data={data} onBack={goLanding}/>;
    if (route.name === 'lowstock') return <window.LowStock data={data} onBack={goLanding}/>;
    if (route.name === 'maint') return <window.Maintenance data={data} onBack={goLanding}/>;
    if (route.name === 'diag') return <window.Diagnostics data={data} onBack={goLanding} tweaks={tweaks}/>;
    ...
    <div className="min-h-screen">
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-2.5 flex items-center justify-between sticky top-0 z-10">
        ...
        <div className="flex items-center gap-1">
          ...
          <NavBtn icon="grid" label="Storage" active={route.name==='storage'} onClick={()=>setRoute({name:'storage'})}/>
          <NavBtn icon="warn" label="Low stock" active={route.name==='lowstock'} onClick={()=>setRoute({name:'lowstock'})}/>
          <NavBtn icon="cpu" label="Maint" active={route.name==='maint'} onClick={()=>setRoute({name:'maint'})}/>
          <NavBtn icon="list" label="Audit" active={route.name==='audit'} onClick={()=>setRoute({name:'audit'})}/>
          <NavBtn icon="shield" label="Readiness" active={route.name==='admin'} onClick={()=>setRoute({name:'admin'})}/>
          <NavBtn icon="printer" label="QR" active={route.name==='labels'} onClick={()=>setRoute({name:'labels'})}/>
          <NavBtn icon="info" label="Diagnostics" active={route.name==='diag'} onClick={()=>setRoute({name:'diag'})}/>
        </div>
      </div>
